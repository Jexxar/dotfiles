...
function LO_kill_older() {
    local bf_proc=( $(ps -aux | grep lightsOn | egrep -v 'grep --color=auto' | sort -u | awk '{print $1"_"$2"_"$9}') )
    local ko_done=false
...
function hasAppStreaming() {
    if wmctrl -l | grep -i "youtube\|vimeo\|anime" ; then 
        echo "yes"
    else
        echo "no"
    fi
...
function checkDelayProgs() {
    ...
    if [ "$isRunning" == "yes" ]; then
        ...
        hasStreaming=$(hasAppStreaming)
        log "hasAppStreaming(): $hasStreaming"
        if [ "$hasStreaming" != "no" ]; then 
            log "checkDelayProgs(): Delaying the screensaver because a streaming (youtube\|vimeo\|anime) is running..."
            delayScreensaver
        fi
    else
        return 0
...
function runcheck() {
    if [[ "$activ_win_title" = *$1* ]]; then
        if [ "$(pidof -s $1)" ]; then
            log "runcheck(): $1 fullscreen detected"
            return 1
        fi
    fi
}
