...
# Translate Lang for action (tooltip only)
actionDescIntl = dict(
    cancel='Cancelar', c='Cancelar',
    lock='Bloquear', b='Bloquear',
    logout='Sair', s='Sair',
    suspend='Suspender', u='Suspender',
    hybridsleep='HybridSleep', y='HybridSleep',
    hibernate='Hibernar', h='Hibernar',
    reboot='Reboot', r='Reboot',
    poweroff='Desligar', d='Desligar'
)
...
if display:
    ...
    class Blexit(BlexitBase):
        ...
        def configure(self):
            if self.config_file:
                ...
                for section in "cancel", "lock", "logout", "suspend", "reboot", "poweroff":
                    self.cp.add_section(section)
    ...
    default_theme_settings = dict(
        ...
        buttonImageLock=BlexitThemeDetail('lock.png', False, 'string'),
    ...
    class BlexitMerlin(Blexit):
        ...
        def construct_ui(self):
            ...
            for button in self.bva:
                (action, label, actionfunc, method, show, onError) = button
                if not show == 0:
                    self.add_button(show, actionfunc, label=label, btype=actionToMethod[action], ttype=actionDescIntl[action])
            self.status = gtk.Label()
        ...
        def add_button(self, show, action, label=None, btype=None, ttype=None):
            ...
            self.tooltips.set_tip(button, str(ttype))
...
def get_options():
    result = None
    import argparse
    parser = argparse.ArgumentParser(description="Openbox exit")
    if display:
        parser.add_argument("-l", "--logout", help="Sair",
                            action="store_true")
    parser.add_argument("-k", "--lock", help="Bloquear",
                        action="store_true")
    parser.add_argument("-s", "--suspend", help="Suspender",
                        action="store_true")
    parser.add_argument("-i", "--hibernate", help="Hibernar",
                        action="store_true")
    parser.add_argument("-y", "--hybridsleep", help="Hybrid sleep",
                        action="store_true")
    parser.add_argument("-b", "--reboot", help="Reboot",
                        action="store_true")
    parser.add_argument("-p", "--poweroff", help="Desligar",
                        action="store_true")
