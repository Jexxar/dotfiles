 ...
 filetype=$(file  "$arg")
   ...
   case "$filetype" in 
        *text*)                    #Any type of Text file
         ...
         choice=`gdialog --menu "How should I process this file($arg)?" 10 50 4 '1' "Print" '2' "Make .ps" '3' "Make .html" '4' "Make .rtf" 2>&1`
        ...
        case $choice in
               ...
               2)   ##print to a .ps file of your choice 
                  NEWNAME=`gdialog --title "Generate Postscript File" --inputbox "Enter Filename (include extension)" 10 50 "$arg".ps 2>&1`
                  enscript -2 -G2rE -U1 --borders --word-wrap -p "$NEWNAME"  "$arg"
               ;;

               3)   ##Generate an .html
                  NEWNAME=`gdialog --title "Generate .html" --inputbox "Enter Filename (include extension)" 10 50 "$arg".html 2>&1`
                  enscript -E --color -w html -p "$NEWNAME"  "$arg"
               ;;

               4)   ##Generate an .rtf
                   NEWNAME=`gdialog --title "Generate .rtf" --inputbox "Enter Filename (include extension)" 10 50 "$arg".rtf 2>&1`     
