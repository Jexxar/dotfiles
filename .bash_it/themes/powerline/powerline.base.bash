...
function __powerline_qtmail_prompt {
    local nQtm=`qt_mail`
    [[ $nQtm -gt 0 ]] && echo "✉ $nQtm|${QTMAIL_THEME_PROMPT_COLOR}"
}

function __powerline_qtjobs_prompt {
    local nQtJob="$(jobs -r | wc -l | sed -e "s/ //g")"
    [[ $nQtJob -gt 0 ]] && echo "🔧 ${nQtJob}|${QTJOB_THEME_PROMPT_COLOR}"
}

function __powerline_in_scr_prompt {
    local sScrPth="/var/run/screen/S-$USER";
    if [ -e "$sScrPth" ]; then
        if [ ! -n "`find $sScrPth -prune -empty`" ]; then
            echo "🖵 $(screen -ls | egrep -c "[0-9]+\.([a-zA-Z0-9\-]+)?\.[a-zA-Z]*")|${INSCR_THEME_PROMPT_COLOR}"
        fi;
    fi
}
